...
flowchart TD
    ...
    Menu --> M4["4. 예약 실행<br/>(윈도우 작업 스케줄러)"]
...
> 전체 다이어그램(레시피 공유·배포 구조·모듈 의존 계층 포함)은 **[`_internal/docs/flowchart.md`](_internal/docs/flowchart.md)**,
...
| **내 브라우저로 공짜 번역** | *진짜 크롬*을 구동하므로, 크롬의 언어별 '항상 번역'을 켜두면 외국어 페이지(러/일/중/아랍 뉴스)가 **번역된 채로** 수집됩니다 — 번역 API·키 0원. 구조는 그대로라 자가치유 무영향. ⚠ 번역 텍스트는 외부 번역 서버로 전송(쿠키는 미전송) — **공개·비민감 페이지에만** 사용([프라이버시 참고](#프라이버시--데이터-주권)) |
| **이미지 필드** | 레코드별 대표 이미지를 구조로 매칭 → **원격 URL + 오프라인 사본** 둘 다 보관 |
| **누적·재현·감사** | CSV 누적(4종 저장방식) · 레시피(CSV) 재현 · `_runs.csv` 감사 · `replay` 일괄 재현 |
| **예약 자동화** | 메뉴에서 Windows 작업 스케줄러 등록/조회/삭제(schtasks 명령 몰라도 됨) — 전체 또는 특정 사이트만, 매시간/매일/매주/한 번만 |
...
| **3. 과거 작업 다시하기** | 성공했던 크롤링을 번호로 골라 입력 없이 재현 |
| **4. 예약 실행** | Windows 작업 스케줄러에 `replay` 재현을 자동 등록/조회/삭제(명령어 몰라도 됨) |
| **5. 레시피** | `읽어들이기`(받은 레시피를 내 URL에 적용·실행) / `공유하기`(마스킹 후 게시판 글쓰기) / `온라인에서 찾기`(공유 게시판 검색) |
| **6. 설정** | LLM 공급자 · 저장/로드 기본값 · 언어(ko/en) · (개발자용) 정합성 점검·역량 매트릭스 |
...
- **자기설명 이름**: 공유 파일명이 `사이트_필드1_필드2…`(예: `웹메일_제목_내용`) — 열어보지 않아도 무엇인지 압니다.
...
- **LLM 없이도 된다는 증거**: 자동 생성되는 [역량 매트릭스](_internal/docs/capabilities.md)는 실제로 가져온
  필드마다 `:V`로 표시합니다. `f1`·`f2`처럼 이름이 일반적인 행은 **LLM 명명을 끈 상태**로 뽑은 결과로,
  이름 자동 부여만 생략됐을 뿐 추출 자체는 성공합니다.
- **자동 push 없음**: 공유는 언제나 *마스킹 → 사람 검수 → 게시판 글쓰기*. 내 검색어가 그대로 새지 않습니다.
- **탐지 최소화**: 차단 사이트는 1페이지·자동 재시도 없음. 무거운 분석은 로컬에 받아둔 HTML로만.
- **크롬 내장 번역을 켠 경우**(위 '공짜 번역' 활용): 번역되는 페이지의 텍스트가 **외부(브라우저 내장) 번역 서버로 전송**
  됩니다(쿠키는 미전송, HTTPS면 요청도 SSL). 소비자용 번역이라 제출 콘텐츠가 익명화되어 번역 제공자
  서비스 개선에 쓰일 수 있습니다(옵트아웃 가능한 기업용 Cloud Translation API와 다름). 이 도구는 **내 로그인
  세션**으로 페이지를 받으므로, 계정 정보가 보이는 페이지에는 쓰지 말고 **공개 뉴스 등 비민감 대상에만**
  자동번역을 적용하세요.
...
```bash
python _internal/tests/run_tests.py     # 전체 테스트 (현재 253개)
```
