...
def _interactive_setup(args):
    ...
    print("  1) auto    " + t("(로그인이 필요 없는 사이트라면 save_as보다 빠르게 수집할 수 있습니다.)")
          + (dflt if d_load == "auto" else ""))
    print("             " + t("(⚠ 다만 로봇임을 숨기지 않으므로, 봇을 차단하는 사이트에서는 접속이 거부될 수 있습니다.)"))
    print("  2) save_as " + t("(내 실제 계정으로 접속하여 로그인이 필요하거나 막힌 사이트에도 대부분 접근할 수 있습니다.)")
          + (dflt if d_load == "save_as" else ""))
    print("             " + t("(⚠ 다만 지나치게 자주 사용하면 해당 계정이 정지(밴)될 위험이 있습니다.)"))
