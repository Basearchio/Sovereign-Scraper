...
DEPENDENCY: requests(있으면) 또는 urllib(폴백) + envfile(최하위 leaf, .env 단일 파서).
...
except ImportError:      # 단독 실행(python services/llm_service.py): _internal 이 경로에 없음 → 추가 후 재시도
    import sys as _sys
    _sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    from envfile import read_env as _read_env_file


def _load_dotenv():
    """프로젝트 루트의 .env 를 os.environ 에 채운다(이미 설정된 값은 유지). 파싱은 envfile 단일 파서."""
