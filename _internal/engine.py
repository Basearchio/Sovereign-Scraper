...
class SelfHealingEngine:
    ...
    def _persist(self):
        ...
        if self.cache_path and self.schema is not None:
            ...
            self.log(f"  -> 스키마 캐시 저장: {self.cache_path}")
    ...
    def calibrate(self, dom) -> Schema:
        ...
        self.log(f"  - row 셀렉터 생성: {schema.row_css}")
        ...
        for name, fdef in FIELD_DEFS.items():
            ...
            for r in rows:                       # 필드가 잡히는 첫 행 사용
                ...
                if node is not None:
                    ...
                    break
            if node is None:
                self.log(f"  - [경고] 필드 '{name}' 위치를 못 찾음")
                continue
            ...
            self.log(f"  - 필드 '{name}' 셀렉터 생성: {fs.css}"
                     f"{f' (@{fs.attr})' if fs.attr else ''}")
    ...
    def build_schema_from_selection(self, rows, sig, selections, dom=None,
        ...
        if mk and dom is not None and len(find_rows_by_marker(dom, mk)) >= self.MIN_ROWS:
            ...
            self.log(f"  - 레코드 경계 마커 사용: {mk} "
                     f"({len(find_rows_by_marker(dom, mk))}개 반복)")
        ...
        for grp in coloc.values():
            ...
            self.log(f"  - 한 노드에 뭉친 {len(derived)}개 필드를 예시 경계로 분리: "
                     f"{list(derived.keys())}")
        ...
        for uname in list(auto_url):
            ...
            if any(other != uname and other not in auto_url
                ...
                self.log(f"  - 중복 링크 컬럼 제거: {uname} (사용자 지정 링크와 동일)")
        self.schema = schema
        self._persist()
        return schema
    ...
    def recalibrate(self, dom, field_names=None, examples=None):
        ...
        if not sels:
            return None
        self.log(f"  [재학습] 로컬 HTML 구조 파악 성공 → {len(sels)}개 필드 재구성: "
                 f"{[n for n, _ in sels]}")
    ...
    def _heal_rows(self, dom):
        ...
        if len(rows) >= self.MIN_ROWS:
            self.log("    · 구조 시그니처 일치 → row 재탐색 성공 (구조 불변)")
        else:
            ...
            s.row_signature = sig
            self.log(f"    · 구조 드리프트 감지 → 반복 블록 재탐지 (새 시그니처={sig})")

        # 새 row 셀렉터 재생성 + 캐시 패치
        old = s.row_css
        s.row_tag, s.row_cls = rows[0].tag, first_class(rows[0])
        s.row_css = css_of(rows[0])
        self.log(f"    · row 셀렉터 자가 갱신: {old}  ->  {s.row_css}")
        return rows
    ...
    def _heal_field(self, name, rows):
        ...
        if node is None:
            self.log(f"    · [실패] 필드 '{name}' 재배치 불가")
            return False

        old = fs["css"]
        fs["tag"], fs["cls"] = node.tag, first_class(node)
        fs["css"] = css_of(node)
        self.log(f"    · 필드 '{name}' 재배치({how}) → 셀렉터 자가 갱신: "
                 f"{old}  ->  {fs['css']}")
        return True
    ...
    def extract(self, dom, base_url=None, local_base=None):
        ...
        # 1) 클래스 가속 경로 시도
            ...
            if len(sig_rows) >= self.MIN_ROWS:
                ...
                if new_css != s.row_css:
                    self.log(f"  [DIFF] row 클래스 셀렉터 '{s.row_css}' 깨짐 "
                             f"→ 구조 시그니처로 재탐색 성공")
                    self.log(f"    · row 셀렉터 자가 갱신: {s.row_css} -> {new_css}")
                    s.row_tag, s.row_cls, s.row_css = rows[0].tag, first_class(rows[0]), new_css
                    healed = True
            else:
                # 3) 구조까지 드리프트 → 반복 블록 전면 재탐지
                self.log(f"  [DIFF] row 구조 변경 감지 → 반복 블록 전면 재탐지")
        ...
        for name in list(s.fields.keys()):
            ...
            if resolved_ok < half(len(rows)):
                ...
                self.log(f"  [DIFF] 필드 '{name}' 위치 추적 실패 "
                         f"({resolved_ok}/{len(rows)}) → 자가 치유")
                ...
                self.log(f"  [DIFF] 필드 '{name}' 클래스 셀렉터 '{fs['css']}' 깨짐 "
                         f"→ 구조 경로로 위치 유지")
                self.log(f"    · 필드 '{name}' 셀렉터 자가 갱신: {fs['css']} -> {new_css}")
        ...
        if healed:
            ...
            self.log("  (빠른 경로 적중 — 치유 불필요, Zero-latency)")
