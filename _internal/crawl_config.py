...
DEPENDENCY: 표준 라이브러리(os)만. engine/cli/llm 무관(leaf).
...
_ENV = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), ".env")  # 루트/.env
...
def _read_env(path=None):
    d = {}
    try:
        with open(path or _ENV, encoding="utf-8") as f:
            for line in f:
                line = line.strip()
