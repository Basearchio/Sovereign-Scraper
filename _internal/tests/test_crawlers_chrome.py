...
- chrome_*: 실제 Save As 는 실사이트 수동 스모크 영역(키입력·창포커스 의존).
...
def test_chrome_surface():
    """[역할] crawlers.chrome 의 공개 진입점(save_as)이 실재하는지.
    (CDP profile attach 경로는 Chrome 136+ 차단으로 제거됨 — 진입점은 save_as 하나.)"""
    import crawlers.chrome as chrome
    assert callable(chrome.chrome_save_as_fetch)
    assert not hasattr(chrome, "chrome_profile_fetch")   # 제거 확정(부활 방지)
...
def test_engine_dropped_chrome_and_block():
    ...
    import engine
    for name in ("chrome_save_as_fetch", "block_reason",
                 "_find_chrome", "_BLOCK_TITLE"):
